...
const DESIRED_FIELDS = ['VIN', 'Make', 'Model', 'ModelYear', 'ErrorCode', 'ErrorText'];
const BATCH_SIZE = 50;
...
const argv = yargs
  ...
  .option('outputs-dir', { demandOption: true, type: 'string' })
  .argv;
...
async function decodeBatchWithRetry(vins, retries = 3, delay = 5000) {
  const formData = new URLSearchParams({ format: 'json', data: vins.join(';') });
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const resp = await axios.post(API_URL, formData, { timeout: 60000 });
      if (!resp.data.Results || !Array.isArray(resp.data.Results)) {
        const errorText = `API Error: 'Results' field missing or not a list. Response: ${JSON.stringify(resp.data).slice(0, 500)}`;
        return vins.map(vin => ({ OriginalVIN: vin, ErrorCode: 'API_BAD_RESPONSE_STRUCTURE', ErrorText: errorText }));
      }
      ...
      if (e.response && e.response.status === 429) {
        ...
        const errorText = e.response ? `HTTP Error: ${e.response.status} - ${e.response.data}` : e.message;
        return vins.map(vin => ({ OriginalVIN: vin, ErrorCode: e.response ? String(e.response.status) : 'REQUEST_ERROR', ErrorText: errorText }));
      }
    }
  }
  const finalErrorText = `Failed to decode batch after ${retries} retries: ${vins.slice(0, 3)}...`;
  return vins.map(vin => ({ OriginalVIN: vin, ErrorCode: 'REQUEST_FAILED_MAX_RETRIES', ErrorText: finalErrorText }));
}
...
(async () => {
  ...
  try {
    ...
    const writer = csvWriter({ path: outputPath, header: DESIRED_FIELDS.map(f => ({ id: f, title: f })) });
    ...
    for (let i = 0; i < numBatches; i++) {
      ...
      results.forEach((item, j) => {
        ...
        const outputRow = { VIN: originalVin, Make: '', Model: '', ModelYear: '', ErrorCode: '0', ErrorText: '' };
        if (item.OriginalVIN) {
          outputRow.ErrorCode = item.ErrorCode || 'UNKNOWN_BATCH_ERR';
          outputRow.ErrorText = item.ErrorText || 'Batch processing error.';
        } else if (Array.isArray(item)) {
          // Not expected for this API, but included for parity
          ...
          if (errorCode && errorCode !== '0') {
            ...
            outputRow.ErrorText = item['Error Text'] || item.AdditionalErrorText || item.Message || '';
          }
        } else {
          outputRow.ErrorCode = 'UNHANDLED_API_VIN_STRUCTURE';
          outputRow.ErrorText = `Unhandled API result structure for VIN ${originalVin}. Type: ${typeof item}, Content: ${JSON.stringify(item).slice(0, 200)}`;
        }
        outputRow.ErrorText = (outputRow.ErrorText || '').trim().replace(/^;+|;+$/g, '');
        allRows.push(Object.fromEntries(DESIRED_FIELDS.map(f => [f, outputRow[f] || ''])));
      });
      processedCount += batchVins.length;
      if (i < numBatches - 1) await new Promise(res => setTimeout(res, DELAY_BETWEEN_BATCHES));
    }
