...
import { deleteFileifExists } from "../lib/jobCleanup";
...
async function cleanupOldFiles() {
  ...
  for (const file of uploadFiles) {
    ...
    if (ageInMinutes > MAX_AGE_MINUTES) {
      console.log(
        `Deleting old uploaded file: ${filePath} (${ageInMinutes.toFixed(
          2
        )} minutes old)`
      );
  ...
  for (const file of outputFiles) {
    ...
    if (ageInMinutes > MAX_AGE_MINUTES) {
      console.log(
        `Deleting old output file: ${filePath} (${ageInMinutes.toFixed(
          2
        )} minutes old)`
      );
      await deleteFileifExists(filePath);
  ...
  for (const file of jobFiles) {
    ...
    if (ageInMinutes > MAX_AGE_MINUTES) {
      console.log(
        `Deleting old job file: ${filePath} (${ageInMinutes.toFixed(
          2
        )} minutes old)`
      );
      await deleteFileifExists(filePath);
