...
jest.mock("@/components/StopProcessButton", () => ({
  StopProcessButton: () => <button>Stop Process</button>,
}));
...
describe("Home page", () => {
  ...
  it("validates and sets file", async () => {
    ...
    if (input) {
      const fileObj = new window.File(["dummy"], "test.xlsx", {
        type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      });
      fireEvent.change(input, { target: { files: [fileObj] } });
    }
    await waitFor(() =>
      expect(screen.getByText(/Process VINs/i)).toBeInTheDocument()
    );
  });
});
